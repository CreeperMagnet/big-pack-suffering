############################################################
# Commands for hitting things with tipped swords
############################################################

data modify storage tcc:storage root.temp.item.potion set from entity @s SelectedItem.tag.tcc.potion.id
execute as @e[predicate=tcc:entity/global.ignore,nbt={HurtTime:10s},distance=0.0000001..15] run function tcc:item/tipped_swords/entity_effects
