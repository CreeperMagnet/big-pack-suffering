...
## Area Effect Clouds
execute as @e[type=area_effect_cloud,tag=!global.ignore,tag=!tcc.aec.no_potion,tag=!tcc.entity] at @s align xyz positioned ~-2 ~ ~-2 if entity @e[type=item,dx=4,dz=4,dy=1,tag=!global.ignore,predicate=tcc:item/unpotioned_item] run function tcc:entity/area_effect_cloud/discern_item
...
## Witch trader check
execute as @e[type=witch,tag=!global.ignore,tag=!global.ignore.kill,predicate=!tcc:entity/in_vehicle] at @s run function tcc:entity/witch/second_clock

## Villager bed exploding
execute as @e[type=villager,predicate=tcc:location/dimension/nether_or_end] if data entity @s SleepingX at @s run function tcc:entity/villager/blow_up
