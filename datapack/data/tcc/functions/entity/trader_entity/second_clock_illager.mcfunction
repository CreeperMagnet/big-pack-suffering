...
execute if entity @s[tag=tcc.enchanter,tag=!tcc.enchanter.casting,predicate=tcc:chance/one_fifth,tag=!tcc.no_ai] if entity @e[team=global.antivil,distance=..10,tag=!tcc.enchanter,tag=!tcc.snail,predicate=tcc:entity/global.ignore,nbt=!{ActiveEffects:[{Ambient:1b}]}] run function tcc:entity/enchanter/spell_start
...
execute if entity @s[tag=tcc.witch_trader,predicate=!tcc:entity/in_vehicle,tag=!tcc.no_ai] unless entity @a[predicate=tcc:entity/wearing/witch_hat,distance=..10] run function tcc:entity/witch/switch_to_witch
